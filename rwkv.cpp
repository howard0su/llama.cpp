...
static const ggml_type FORMAT_TYPE_TO_GGML_TYPE[] = {
    GGML_TYPE_F32,
    GGML_TYPE_F16,
    GGML_TYPE_Q4_0,
    GGML_TYPE_Q4_1
...
struct rwkv_context * rwkv_init_from_file(const char * file_path, uint32_t n_threads) {
    ...
    RWKV_ASSERT_NULL(emb->ne[0] == model->n_embed, "Unexpected dimension of embedding matrix %I64d", emb->ne[0]);
    RWKV_ASSERT_NULL(emb->ne[1] == model->n_vocab, "Unexpected dimension of embedding matrix %I64d", emb->ne[1]);
...
static void filter_outliner(std::vector<float> &data, float zscore)
{
    double mean = 0.0;
    double stddev = 0.0;
    ...
    for (int i = 0; i < data.size(); i++)
    {
        stddev += pow(data.at(i) - mean, 2);
    }
    
    stddev = sqrt(stddev / (data.size() - 1));

    // calculate lower and upper bounds of zscore
    float lower_bound = mean - zscore * stddev;
    float upper_bound = mean + zscore * stddev;
...
bool rwkv_model_quantize(const char * model_file_path_in, const char * model_file_path_out, uint32_t q_type) {
    ...
    // Process parameters
    {
        ...
        while (true) {
            ...
            if (quantize) {
                ...
                filter_outliner(data_f32, 4.32f); // 3.29 for 99.9%, 4.32 for 99.99%
